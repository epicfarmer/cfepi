...
namespace detail {
  ...
  TEST_CASE("[sir_generator] SIR model works modularly") {
    ...
    auto always_true_event
        = [](const auto &param __attribute__((unused)), const auto &state __attribute__((unused)),
    ...
    cfepi::run_simulation<sir_epidemic_states, any_sir_event_type, any_sir_event>(
        cfepi::all_event_types<any_sir_event_type>{}, initial_conditions,
  ...
  */
  ...
  TEST_CASE("[sample_view] sample_view is working") {
    ...
    for (auto it1 = std::begin(view1); (it1 != std::end(view1)) && (it2 != std::end(view2));
         ++it1) {
      CHECK((*it1) == (*it2));
      ++it2;
    }
    it2 = std::begin(view2);
    for (auto it1 = std::begin(view3); (it1 != std::end(view3)) && (it2 != std::end(view2));
         ++it1) {
      CHECK((*it1) == (*it2));
      ++it2;
    }
  ...
  */
  ...
  typedef std::variant<seir_recovery_event_type, seir_infection_event_type,
                       seir_exposure_event_type>
      any_seir_event_type;
  ...
  TEST_CASE("[sir_generator] SEIR model works modularly") {
    ...
    cfepi::run_simulation<seir_epidemic_states, any_seir_event_type, any_seir_event>(
        cfepi::all_event_types<any_seir_event_type>{}, initial_conditions,
    ...
    auto filter_by_infected =
        [&counts_to_filter_to](
          ...
          auto incidence_counts
              = cfepi::aggregate_state(param).potential_state_counts[1 << seir_epidemic_states::I];
  ...
  */
  ...
  struct map_sir_recovery_event_type
      : public cfepi::transition_event_type<map_sir_epidemic_states> {
  ...
  TEST_CASE("[sir_generator] map based SIR model works") {
    ...
    cfepi::run_simulation<map_sir_epidemic_states, any_map_sir_event_type, any_map_sir_event>(
        cfepi::all_event_types<any_map_sir_event_type>{}, initial_conditions,
        std::array<double, 2>({.1, 2. / static_cast<double>(population_size)}),
        {std::make_tuple(always_true_event, always_true_state, do_nothing),
         std::make_tuple(always_true_event, always_true_state, do_nothing)});
  }
  ...
  struct config_map_sir_recovery_event_type
      : public cfepi::transition_event_type<config_map_sir_epidemic_states> {
  ...
  TEST_CASE("[sir_generator] config based SIR model works without config") {
    ...
    auto initial_conditions = cfepi::default_state<config_map_sir_epidemic_states>(
        config_map_sir_epidemic_states{}["S"], config_map_sir_epidemic_states{}["I"],
        population_size, 1UL);
    ...
    auto flat_reduction
        = [](const auto &param __attribute__((unused)), const auto &state __attribute__((unused)),
            ...
            if (param.index() == event_index) {
              if (dist(rng) < reduction_percentage) {
                return (false);
              }
            }
            return (true);
    ...
    auto strict_incidence_filter = [](const auto &setup, const auto &new_state,
                                      std::default_random_engine &rng __attribute__((unused))) {
      ...
      if (this_time < simulation_length) {
        auto incidence_counts = cfepi::aggregate_state(setup.states_entered)
                                    .potential_state_counts[1 << compartment_to_filter];
        return (incidence_counts == counts_to_filter_to[this_time]);
      }
      return (true);
    };
    ...
    cfepi::filtration_setup<config_map_sir_epidemic_states, any_config_map_sir_event_type>{
        initial_conditions, always_true_event, always_true_state, do_nothing};
    cfepi::run_simulation<config_map_sir_epidemic_states, any_config_map_sir_event_type,
                          any_config_map_sir_event>(
        cfepi::all_event_types<any_config_map_sir_event_type>{}, initial_conditions,
        std::array<double, 2>({.1, 2. / static_cast<double>(population_size)}),
  ...
  TEST_CASE("[sir_generator] config based SIR model works") {
    const static constexpr std::string_view json_config
        = "{    \"states\": [\"S\", \"I\", \"R\", \"V\"],    \"events\":    [	{	    "
          ...
          "],	    \"comment_rate\": \"1.05 * .44444444444444 / 10000\",	    \"rate\": "
          "0.00004666666667	}    ],    \"initial_conditions\": {	\"S\": 9999,	\"I\": 1   "
          " },    \"worlds\": [	{	    \"event_filter\": { \"function\": \"do_nothing\" },	   "
          " \"state_modifier\": { \"function\": \"do_nothing\" },	    \"state_filter\": {	"
          "	\"function\": \"strict_incidence_filter\",		\"parameters\" : {	"
          "	    \"compartment\" : \"I\",		    \"counts\": [		       "
          "1,		       2		     ]		}	    }	},	{	   "
          " \"event_filter\": {		\"function\": \"flat_reduction\",		"
          "\"parameters\" : {		    \"event_index\": 0,		    "
          "\"reduction_percentage\": 1.0		}	    },	    \"state_modifier\": "
          "{		\"function\": \"single_time_move\",		\"parameters\" : {	"
          "	    \"percent_to_move\": 0.24,		    \"source_compartments\": [\"S\"],	"
  ...
  TEST_CASE("[seir_generator] config based SEIR model works") {
    const static constexpr std::string_view json_config
        = "{    \"states\": [\"S\", \"E\", \"I\", \"R\", \"V\"],    \"events\":    [	{	   "
          " \"source\": [ [\"E\"] ],	    \"destination\": [ {\"index\": 0, \"compartment\": "
          "\"I\"} ],	    \"rate\": 0.444444	},	{	    \"source\": [ [\"I\"] ],	   "
          " \"destination\": [ {\"index\": 0, \"compartment\": \"R\"} ],	    \"rate\": "
          "0.444444	},	{	    \"source\": [ [ \"I\" ], [ \"S\", \"V\" ] ],	   "
          " \"destination\": [ {\"index\": 1, \"compartment\": \"E\"} ],	    "
          "\"comment_rate\": \"1.05 * .44444444444444 / 10000\",	    \"rate\": "
          "0.00004666666667	}    ],    \"initial_conditions\": {	\"S\": 9999,	\"I\": 1   "
          " },    \"worlds\": [	{	    \"event_filter\": { \"function\": \"do_nothing\" },	   "
          " \"state_modifier\": { \"function\": \"do_nothing\" },	    \"state_filter\": { "
          "\"function\": \"do_nothing\" }	},	{	    \"event_filter\": {		"
          "\"function\": \"flat_reduction\",		\"parameters\" : {		    "
          "\"event_index\": 0,		    \"reduction_percentage\": 1.0		}	   "
          " },	    \"state_modifier\": {		\"function\": \"single_time_move\",	"
          "	\"parameters\" : {		    \"percent_to_move\": 0.24,		    "
          "\"source_compartments\": [\"S\"],		    \"destination_compartment\": \"V\",	"
          "	    \"time\": 0		}	    },	    \"state_filter\": { \"function\": "
          "\"do_nothing\" }	}    ]}";
    cfepi::run_simulation_from_config<json_config>();
  }
