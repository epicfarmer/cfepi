...
namespace cfepi {
  ...
  concept is_sized_enum
      = requires(enum_type e) {
          { std::size(e) } -> std::unsigned_integral;
        } && std::same_as<typename enum_type::state, typename enum_type::state> && std::default_initializable<enum_type>;  //&&std::is_enum<typename enum_type::state>::value;
  //! @}
  ...
  struct sir_state {
    ...
    sir_state operator||(const sir_state &other) const {
      ...
      for (auto elem : std::ranges::views::zip(this->potential_states, other.potential_states,
                                               std::ranges::views::iota(0UL))) {
        rc.potential_states[std::get<2>(elem)] = std::get<0>(elem) | std::get<1>(elem);
      }

      /*
      std::ranges::views::transform(this->potential_states,
    ...
    size_t size() const { return (std::size(potential_states)); }
  ...
  //! \brief Class for keeping track of the current state of a compartmental model (without
  //! potential states).
  //!
  ...
  template <typename states_t> struct aggregated_sir_state {
    //! \brief The main part of the class. For each element of the powerset of states_t, stores
    //! counts of people in that potential states
    ...
    bool operator==(const aggregated_sir_state<states_t> &other) const {
      return (std::transform_reduce(
          std::begin(potential_state_counts), std::end(potential_state_counts),
          std::begin(other.potential_state_counts), true,
  ...
  concept is_event_type_like
      = is_sized_enum<typename T::state_type>
        && requires(T e, size_t i) {
             { e.preconditions[i][i] } -> std::convertible_to<bool>;
             {
               e.postconditions[i]
               } -> std::convertible_to<std::optional<typename T::state_type::state>>;
             { std::size(e) } -> std::unsigned_integral;
           };
  ...
  struct sir_event {
    ...
    sir_event(const event_type &_type) : type(_type) {}
  };
  ...
  template <typename states_t> struct interaction_event_type : public sir_event_type<states_t, 2> {
    ...
    constexpr interaction_event_type(
        ...
        : sir_event_type<states_t, 2>(
            std::array<std::bitset<std::size(states_t{})>, 2>{preconditions_for_first_person,
  ...
  template <typename states_t> struct transition_event_type : public sir_event_type<states_t, 1> {
    ...
    constexpr transition_event_type(const std::bitset<std::size(states_t{})> &_preconditions,
        ...
        : sir_event_type<states_t, 1>(
            std::array<std::bitset<std::size(states_t{})>, 1>{_preconditions},
  ...
  template <typename states_t> struct any_event_apply_entered_states {
    ...
    void operator()(const auto &x) const {
      ...
      for (auto person_index :
           std::ranges::views::iota(0UL) | std::ranges::views::take(event_size)) {
  ...
  struct check_event_precondition_enumerated {
    ...
    auto operator()(const auto &x) {  // x is a potential_states
      auto rc = (std::get<0>(x) & event.preconditions[precondition_index]).any();
      return (rc);
    }
  };
  ...
  const auto get_precondition_satisfying_indices(const auto &event,
    ...
    auto range_to_contstruct_vector
        = std::ranges::views::zip(current_state.potential_states, std::ranges::views::iota(0UL))
          | std::ranges::views::filter(
              check_event_precondition_enumerated<states_t, decltype(event), precondition_index>{
                  event})
          | std::ranges::views::transform([](const auto &x) { return (std::get<1>(x)); });
    std::ranges::copy(std::begin(range_to_contstruct_vector), std::end(range_to_contstruct_vector),
                      std::back_inserter(return_value));
    return return_value;
  }

  template <typename event_type_t> const auto transform_array_to_sir_event_l
      = [](const event_type_t &event_type, const auto &x) {
          sir_event<event_type_t> rc{event_type};
  ...
  */
  ...
  template <typename states_t> void print(const sir_state<states_t> &state,
                                          const std::string &prefix = "", bool aggregate = true) {
    if (aggregate) {
  ...
  void print(const aggregated_sir_state<states_t> &aggregates, const std::string &prefix = "") {
    ...
    for (size_t subset = 0; subset <= detail::int_pow(2, std::size(states_t{})); ++subset) {
      if (aggregates.potential_state_counts[subset] > 0) {
        std::cout << prefix;
        for (size_t compartment = 0; compartment < std::size(states_t{}); ++compartment) {
          if ((subset % detail::int_pow(2, compartment + 1) / detail::int_pow(2, compartment))
              == 1) {
  ...
  template <typename states_t> std::tuple<
      std::vector<std::string>,
      std::tuple<std::vector<std::string>, std::vector<epidemic_time_t>, std::vector<size_t>>>
  convert_to_dataframe(aggregated_sir_state<states_t> aggregates, states_t states) {
    // One column for time, and one for each state
    std::vector<std::string> names{"compartment", "time", "count"};
    auto columns = std::make_tuple(std::vector<std::string>{}, std::vector<epidemic_time_t>{},
                                   std::vector<size_t>{});
    for (size_t subset = 0; subset <= detail::int_pow(2, std::size(states_t{})); ++subset) {
      if (aggregates.potential_state_counts[subset] > 0) {
        std::string all_compartments = "";
        for (size_t compartment = 0; compartment < std::size(states_t{}); ++compartment) {
          if ((subset % detail::int_pow(2, compartment + 1) / detail::int_pow(2, compartment))
              == 1) {
            all_compartments += states[compartment];
          }
        }
        std::get<0>(columns).push_back(all_compartments);
        std::get<1>(columns).push_back(aggregates.time);
        std::get<2>(columns).push_back(aggregates.potential_state_counts[subset]);
      }
    }
    return std::make_tuple(names, columns);
  }
  ...
  concatenate_dataframes(
			 std::vector<std::vector<aggregated_sir_state<states_t>>>
          input_frames,
			 std::string index_column_name_inner, states_t states) {
    std::vector<std::string> names{};
    std::tuple<std::vector<std::string>, std::vector<epidemic_time_t>, std::vector<size_t>,
               std::vector<size_t>>
        columns{};
    if(std::size(input_frames) == 0) {
      return std::make_tuple(names, columns);
    }
    auto outer_it = std::begin(input_frames);
    while((outer_it != std::end(input_frames)) && (std::size(*outer_it) == 0)) {continue;};
    if (outer_it == std::end(input_frames)) {
      return std::make_tuple(names, columns);
    }
    std::tuple< std::vector<std::string>, std::tuple<std::vector<std::string>, std::vector<epidemic_time_t>, std::vector<size_t>>> sample_frame = convert_to_dataframe((*outer_it)[0], states);
    names = std::get<0>(sample_frame);
    names.push_back(index_column_name_inner);

    size_t expected_size = 0UL;
    for (auto outer_slice : input_frames) {
      for (auto inner_slice : outer_slice) {
	expected_size += std::size(std::get<0>(std::get<1>(convert_to_dataframe(inner_slice, states))));
      }
    }
    std::get<0>(columns).reserve(expected_size);
    std::get<1>(columns).reserve(expected_size);
    std::get<2>(columns).reserve(expected_size);
    std::get<3>(columns).reserve(expected_size);

    for (auto outer_slice : input_frames) {
      size_t inner_counter = 0UL;
      for (auto inner_slice : outer_slice) {
	const auto inner_columns = std::get<1>(convert_to_dataframe(inner_slice, states));
	std::vector<size_t> column_2_copy = std::get<2>(inner_columns);
	std::fill(std::begin(column_2_copy), std::end(column_2_copy), inner_counter);
	std::get<0>(columns).insert(std::end(std::get<0>(columns)), std::begin(std::get<0>(inner_columns)), std::end(std::get<0>(inner_columns)));
	std::get<1>(columns).insert(std::end(std::get<1>(columns)), std::begin(std::get<1>(inner_columns)), std::end(std::get<1>(inner_columns)));
	std::get<2>(columns).insert(std::end(std::get<2>(columns)), std::begin(std::get<2>(inner_columns)), std::end(std::get<2>(inner_columns)));
	std::get<3>(columns).insert(std::end(std::get<3>(columns)), std::begin(column_2_copy), std::end(column_2_copy));
	++inner_counter;
      }
    }
    return std::make_tuple(names, columns);
  ...
  struct any_event<any_event_type, std::index_sequence<event_index...>> {
    typedef std::variant<sir_event<std::variant_alternative_t<event_index, any_event_type>>...>
        type;
  };
  ...
  struct single_type_event_generator<event_type_t, std::index_sequence<precondition_index...>> {
    ...
    auto cartesian_range() {
      return (std::apply(std::ranges::views::cartesian_product, vectors));
    ...
    explicit single_type_event_generator(
        event_type_t event_type_, const sir_state<typename event_type_t::state_type> &current_state)
        : event_type(event_type_),
          vectors(
              std::make_tuple(get_precondition_satisfying_indices<typename event_type_t::state_type,
                                                                  precondition_index>(
                  event_type, current_state)...)) {
      // This size check is conditioned on the cartesian product of a non-empty range and an empty
      // range segfaulting There is a commented out test called "Product view works for a non-empty
      // and empty vector", which verifies this is still happening
      std::vector<size_t> all_vector_sizes = std::apply(
          [](const auto &...x) { return (std::vector<size_t>{std::size(x)...}); }, vectors);
