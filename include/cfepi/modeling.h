  ...
  const size_t seed)
{
  ...
  auto sampled_events_by_setup_view = std::ranges::views::cartesian_product(setup_index_range, all_sampled_events_view);

  auto filtered_events_by_setup_view = std::ranges::views::filter(sampled_events_by_setup_view,
  ...
  auto &simulation_seed)
{
  ...
  auto range_to_construct_vector_from = std::ranges::views::transform(setups_by_filter, update_lambda);
  std::ranges::copy(std::begin(range_to_construct_vector_from), std::end(range_to_construct_vector_from), std::back_inserter(states_next));
  ...
  auto &resets)
{
  ...
  auto range_to_construct_vector_from = std::ranges::views::transform(setups_by_filter, [](const auto &x) { return (aggregate_state(x.current_state)); });
  std::ranges::copy(std::begin(range_to_construct_vector_from), std::end(range_to_construct_vector_from), std::back_inserter(return_value));
  return(return_value);
}
