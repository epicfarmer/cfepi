...
public:
  ...
  constexpr size_t operator[](std::string_view s) const
  {
    ...
    return (static_cast<size_t>(std::distance(
      std::begin(state_array), std::find_if(std::begin(state_array), std::end(state_array), the_lambda))));
  }
...
{
  return (std::get<1>(daw::json::json_details::find_range<NoCommentSkippingPolicyChecked>(json, elem)));
}
...
{
  auto sub_json = std::get<1>(daw::json::json_details::find_range<NoCommentSkippingPolicyChecked>(json, "source"));
...
constexpr auto parse_json_event_type_source(states_t states, std::string_view json)
{
  ...
  auto inner_lambda = [&states, &bitset_int](
                        const auto &this_state) { bitset_int = bitset_int + (1UL << states[this_state]); };
...
constexpr auto parse_json_event_type_all_sources(states_t states, std::string_view json)
{
  ...
  std::vector<std::string_view> sources_vec{ from_json_array<json_delayed<no_name, std::string_view>>(json) };
  std::transform(std::begin(sources_vec), std::end(sources_vec), std::begin(rc), [&states](const auto &x) {
...
constexpr auto parse_json_event_type_destination(states_t states, std::string_view json)
{
  ...
  std::vector<std::string_view> destination_vec{ from_json_array<json_delayed<no_name, std::string_view>>(json) };
  ...
  std::transform(
    std::begin(destination_vec), std::end(destination_vec), std::begin(compartments), [](std::string_view x) {
      return (from_json<std::string_view>(x, "compartment"));
    });

  std::vector<size_t> indices{};
  indices.resize(destination_size);
  std::transform(std::begin(destination_vec), std::end(destination_vec), std::begin(indices), [](std::string_view x) {
    return (from_json<size_t>(x, "index"));
  });

  std::vector<std::string_view> rc_tmp __attribute__((unused)){};
  std::ranges::for_each(
    std::ranges::views::iota(0UL, std::size(indices)), [&states, &rc, &indices, &compartments](const auto &index) {
      rc[indices[index]] = std::make_optional(states[compartments[index]]);
    });
  return (rc);
};
...
template<typename states_t, size_t event_type_size>
requires cfepi::is_sized_enum<states_t>
constexpr cfepi::sir_event_type<states_t, event_type_size> parse_json_event_type(states_t states, std::string_view json)
{
  ...
  std::string_view destination_json =
    std::get<1>(daw::json::json_details::find_range<NoCommentSkippingPolicyChecked>(json, "destination"));
...
constexpr std::array<size_t, num_events> parse_json_array_event_type_sizes(std::string_view json)
{
  ...
  std::vector<std::string_view> events_vec{ from_json_array<json_delayed<no_name, std::string_view>>(json) };
...
constexpr std::array<double_type, num_events> parse_json_array_event_type_rates(std::string_view json)
{
...
template<typename states_t, auto event_sizes, typename = std::make_index_sequence<std::size(event_sizes)>>
struct parse_json_array_event_type_struct;

template<typename states_t, auto event_sizes, size_t... event_sizes_index>
struct parse_json_array_event_type_struct<states_t, event_sizes, std::index_sequence<event_sizes_index...>>
{
  constexpr std::tuple<cfepi::sir_event_type<states_t, event_sizes[event_sizes_index]>...> parse(states_t states,
    std::string_view json)
  {
    auto sub_jsons = from_json_array<json_delayed<no_name, std::string_view>>(json);
    auto rc = std::make_tuple(
      parse_json_event_type<states_t, event_sizes[event_sizes_index]>(states, sub_jsons[event_sizes_index])...);
    return (rc);
  }
  typedef std::variant<cfepi::sir_event_type<states_t, event_sizes[event_sizes_index]>...> any_event_type;
};
  ...
  parse_json_event_filter_flat_reduction_single_compartment(std::string_view json, states_t states)
{
  ...
  return [reduction_percentage, event_index, index_to_filter, compartment_to_filter](const any_event &event,
           const cfepi::sir_state<states_t> &state __attribute__((unused)),
           std::default_random_engine &rng) {
    std::uniform_real_distribution<> dist(0.0, 1.0);
    if (event.index() == event_index) {
  ...
  parse_json_state_filter(std::string_view json, states_t states)
{
  ...
  if (function_name == "strict_incidence_filter") {
    return parse_json_state_filter_strict_incidence<states_t, any_event>(parse_json_select(json, "parameters"), states);
  }
  ...
  parse_json_state_modifier(std::string_view json, states_t states)
{
  ...
  if (function_name == "single_time_move") {
    return parse_json_state_modifier_single_move<states_t>(parse_json_select(json, "parameters"), states);
  }
  ...
  parse_json_state_modifier_single_move(std::string_view json, states_t states)
{
  ...
  auto from_compartments = ranges::to<std::vector>(
    std::ranges::views::transform(from_compartments_string, [states](const auto &x) { return (states[x]); }));
...
{
  return std::make_tuple(parse_json_event_filter<states_t, any_event>(parse_json_select(json, "event_filter"), states),
...
void run_simulation_from_config(epidemic_time_t epidemic_length [[maybe_unused]] = 365, size_t simulation_seed [[maybe_unused]] = 2)
{
  /*
