...
&&std::same_as<typename enum_type::state, typename enum_type::state>
...
struct sir_state {
  ...
  sir_state operator||(const sir_state &other) const {
    ...
    for (auto elem : std::ranges::views::zip(this->potential_states, other.potential_states, std::ranges::views::iota(0UL))) {
...
concept is_event_type_like =
  is_sized_enum<typename T::state_type> && requires(T e, size_t i) {
...
const auto get_precondition_satisfying_indices(const auto &event,
  ...
  std::ranges::copy(std::begin(range_to_contstruct_vector), std::end(range_to_contstruct_vector), std::back_inserter(return_value));
  return return_value;
}
